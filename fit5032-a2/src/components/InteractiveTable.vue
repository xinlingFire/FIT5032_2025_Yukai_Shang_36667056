...
const filteredRows = computed(() => {
  ...
  return props.rows.filter((row) => {
    const matchesAllColumns = !query || props.columns.some((column) => String(row[column.key] ?? '').toLocaleLowerCase().includes(query))
    const matchesColumnFilters = props.columns.every((column) => {
      const columnQuery = String(columnSearch.value[column.key] ?? '').trim().toLocaleLowerCase()
      return !columnQuery || String(row[column.key] ?? '').toLocaleLowerCase().includes(columnQuery)
    })
    return matchesAllColumns && matchesColumnFilters
  })
})
...
<template>
  <section class="interactive-table" :aria-label="title">
    ...
    <div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th v-for="column in columns" :key="column.key" scope="col"><button class="sort-button" type="button" @click="sort(column.key)">{{ column.label }} <span aria-hidden="true">{{ sortKey === column.key ? (direction === 'asc' ? '↑' : '↓') : '↕' }}</span></button></th></tr><tr class="column-search-row"><th v-for="column in columns" :key="`${column.key}-search`" scope="col"><label><span class="visually-hidden">Search {{ column.label }}</span><input v-model="columnSearch[column.key]" class="form-control form-control-sm" type="search" :placeholder="`Search ${column.label}`" @input="resetPage" /></label></th></tr></thead><tbody><tr v-for="row in visibleRows" :key="row.id"><td v-for="column in columns" :key="column.key">{{ row[column.key] }}</td></tr><tr v-if="!visibleRows.length"><td :colspan="columns.length">No matching records.</td></tr></tbody></table></div>
